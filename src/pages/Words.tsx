...
function WordsTable(): JSX.Element {
  ...
  const showChordedColumns = words.some((word) => word.chordHighest || word.chordLastPractice)
  ...
  words.sort((a, b) => {
    ...
    switch (sortBy) {
      ...
      case Columns.ChordHighestWpmDatetime:
        result = (a.chordHighest?.date.getTime() ?? 0) - (b.chordHighest?.date.getTime() ?? 0)
        break
      case Columns.ChordLastPracticeWpm:
        result = (a.chordLastPractice?.wpm ?? 0) - (b.chordLastPractice?.wpm ?? 0)
        break
      case Columns.ChordLastPracticeWpmDatetime:
        result = (a.chordLastPractice?.date.getTime() ?? 0) - (b.chordLastPractice?.date.getTime() ?? 0)
        break
    }
    if (order === Order.DESC) {
      result = 0 - result
    ...
    <div className="table-container grow overflow-y-auto ">
      <table className="w-full table-fixed">
        <thead className="top-0 sticky bg-blue-950 drop-shadow-lg text-lg">
          <tr>
            <th className="p-3 pl-10 text-left font-bold px-2 cursor-pointer" onClick={() => setSort(Columns.Word)}>
              Word
            </th>
            <th className="font-bold px-2 text-right cursor-pointer" onClick={() => setSort(Columns.HighestWpm)}>
              Record
            </th>
            <th className="font-bold px-2 text-left cursor-pointer" onClick={() => setSort(Columns.HighestWpmDatetime)}>
              Date
            </th>
            <th className="font-bold px-2 text-right cursor-pointer" onClick={() => setSort(Columns.LastPracticeWpm)}>
              Last Practice
            </th>
            <th
        ...
        <tbody>
          {words.map((word) => (
            <tr className="hover:bg-slate-700" key={word.word}>
              ...
              <td className="px-2 text-gray-400">{formattedDate(word.highest.date)}</td>
              <td className="text-right px-2">{word.lastPractice.wpm.toFixed(1)}</td>
              <td className="px-2 text-gray-400">{formattedDate(word.lastPractice.date)}</td>
              {showChordedColumns && (
                <>
                  <td className="text-right px-2">{(word.chordHighest?.wpm ?? 0).toFixed(1)}</td>
                  <td className="px-2 text-gray-400">
                    {word.chordHighest ? formattedDate(word.chordHighest.date) : ""}
                  </td>
                  <td className="text-right px-2">{(word.chordLastPractice?.wpm ?? 0).toFixed(1)}</td>
                  <td className="px-2 text-gray-400">
                    {word.chordLastPractice ? formattedDate(word.chordLastPractice.date) : ""}
                  </td>
                </>
              )}
